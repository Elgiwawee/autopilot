...
def learn_from_outcome(plan, execution: ActionExecution, utilization, decision):
    """
    Capture the result of an executed action and store:

    1️⃣ Learning record (ActionOutcome)
    2️⃣ ML Feature snapshot (Feature Store)
    """
    ...
    try:
        ...
        outcome = ActionOutcome.objects.create(
            ...
            estimated_savings=float(
                getattr(plan, "estimated_monthly_savings", 0)
            ),
            success=success,
        ...
        logger.info(
            "Learning record stored for resource %s",
            resource.resource_id,
        )

        # 2️⃣ Store ML feature snapshot
        ...
        logger.exception(
            "Learning engine failed while recording outcome"
        )

    # 3️⃣ Trigger retraining check
